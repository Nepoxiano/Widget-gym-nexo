...
export default function AdminWidget() {
  ...
  useEffect(() => {
    // Initialize Grist with Full access since Admin needs to write to tables
    connector.ready({ requiredAccess: 'full' });
    loadAllData();
  }, []);

  async function loadAllData() {
    const data = await connector.fetchAllAdminData();
    setAlumnos(data.alumnos);
    setEjercicios(data.ejercicios);
    setRutinasPredefinidas(data.rutinasPredefinidas);
    setRutinasAlumnos(data.rutinasAlumnos);
    
    // Default the exercise dropdown to the first exercise if available
    if (data.ejercicios.length > 0) {
      setNewExerciseForm(prev => ({ ...prev, Ejercicio: data.ejercicios[0].Nombre }));
    }
  }
  ...
  const handleUpdateProfile = async (e) => {
    ...
    if (success) {
      ...
      alert('Error al guardar los cambios en Grist.');
    }
  };
  ...
  const handleAddStudent = async (e) => {
    ...
    const success = await connector.addRecords('Alumnos', [newStudentForm]);
    if (success) {
      ...
      alert('Error al agregar el alumno en Grist.');
    }
  };
  ...
  const handleApplyTemplate = async (templateName) => {
    ...
    // Filter template exercises
    const templateExercises = rutinasPredefinidas.filter(r => r.Plan === templateName);
    if (templateExercises.length === 0) {
      alert('No se encontraron ejercicios en la plantilla seleccionada.');
      return;
    }

    // Prepare records for insertion
    const newRoutineRecords = templateExercises.map((te, idx) => {
      // Grist links rows by references, we store student ID and other fields
      return {
        AlumnoID: selectedAlumno.ID,
        // Map template day representation to student day
        // E.g., "Día 1 - Pecho..." -> "Día 1 - Pecho..."
        Dia: te.Dia,
        Ejercicio: te.Ejercicio,
        Series_Repeticiones: te.Series_Repeticiones,
        Orden: te.Orden,
        Notas: ''
      };
    });

    const success = await connector.addRecords('Rutinas_Alumnos', newRoutineRecords);
    if (success) {
      loadAllData();
      alert('Plantilla cargada correctamente.');
    } else {
      alert('Error al guardar la rutina en Grist.');
    }
  };
  ...
  const handleAddExercise = async (e) => {
    ...
    const success = await connector.addRecords('Rutinas_Alumnos', [newRecord]);
    if (success) {
  ...
  const handleDeleteExercise = async (id) => {
    if (window.confirm('¿Borrar este ejercicio de la rutina?')) {
      const success = await connector.deleteRecords('Rutinas_Alumnos', [id]);
      if (success) {
        loadAllData();
      }
    }
  ...
  const handleSaveExerciseEdit = async (e) => {
    e.preventDefault();
    const success = await connector.updateRecords('Rutinas_Alumnos', [
      { id: editingExerciseId, fields: editExerciseForm }
    ]);
    if (success) {
              ...
              filteredAlumnos.map(student => {
                ...
                return (
                  <div
                    ...
                    style={{
                      padding: '1rem',
                      background: isSelected ? 'rgba(138, 43, 226, 0.1)' : 'rgba(255, 255, 255, 0.02)',
                  ...
                  >
                    <div>
                      <strong style={{ fontSize: '1.05rem', color: isSelected ? 'var(--secondary)' : 'var(--text-primary)' }}>
                      ...
                      {student.Lesionado === 'Sí' && (
                        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.25rem', marginTop: '0.3rem', fontSize: '0.75rem', color: '#ff5252', background: 'rgba(255,23,68,0.1)', padding: '0.1rem 0.4rem', borderRadius: '4px' }}>
                          ⚠️ Lesión: {student.DetalleLesion || 'Sí'}
                        </div>
                      )}
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem', alignItems: 'flex-end' }}>
                      <span className={`badge ${student.Estado === 'Alta' ? 'badge-student' : 'badge-admin'}`}>
                        {student.Estado}
                      </span>
        ...
        <div className="space-y">
          {selectedAlumno ? (
            <div className="space-y fade-in">
                ...
                {!isEditingProfile ? (
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', fontSize: '0.9rem' }}>
                    <div>
                      ...
                      <p style={{ fontWeight: '500', marginBottom: '0.8rem' }}>{selectedAlumno.Edad || 'No especificado'} años</p>
                      
                      <p style={{ color: 'var(--text-muted)' }}>HORARIO</p>
                      <p style={{ fontWeight: '500', marginBottom: '0.8rem' }}>{selectedAlumno.Horario}</p>

                      <p style={{ color: 'var(--text-muted)' }}>DEPORTES QUE PRACTICA</p>
                      <p style={{ fontWeight: '500', marginBottom: '0.8rem' }}>{selectedAlumno.Deportes || 'Ninguno'}</p>
                    </div>
                    <div>
                      <p style={{ color: 'var(--text-muted)' }}>ENFERMEDADES O PATOLOGÍAS</p>
                      <p style={{ fontWeight: '500', color: selectedAlumno.Enfermedades ? 'var(--warning)' : 'var(--text-primary)', marginBottom: '0.8rem' }}>
                        {selectedAlumno.Enfermedades || 'Ninguna'}
                      </p>

                      <p style={{ color: 'var(--text-muted)' }}>RESTRICCIONES MÉDICAS</p>
                      <p style={{ fontWeight: '500', color: selectedAlumno.RestriccionesMedicas ? '#ff5252' : 'var(--text-primary)', marginBottom: '0.8rem' }}>
                  ...
                  <form onSubmit={handleUpdateProfile} className="space-y">
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                      <div className="form-group">
                        <label>Nombre</label>
                        <input type="text" className="input-control" value={editForm.Nombre || ''} onChange={e => setEditForm({...editForm, Nombre: e.target.value})} />
                      </div>
                      <div className="form-group">
                        <label>Apellido</label>
                        <input type="text" className="input-control" value={editForm.Apellido || ''} onChange={e => setEditForm({...editForm, Apellido: e.target.value})} />
                      </div>
                    </div>
              ...
              <div className="glass-panel" style={{ padding: '1.25rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', flexWrap: 'wrap', gap: '0.5rem' }}>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    <button className="btn btn-secondary" style={{ padding: '0.4rem 0.8rem', fontSize: '0.8rem' }} onClick={() => setIsAddingExercise(!isAddingExercise)}>
                      {isAddingExercise ? 'Cancelar' : '+ Agregar Ejercicio'}
                    </button>
                ...
                {studentRoutine.length === 0 && !isAddingExercise && (
                  <div style={{ background: 'rgba(255,255,255,0.01)', padding: '2rem 1rem', border: '1px dashed var(--border)', borderRadius: 'var(--radius-md)', textAlign: 'center', marginBottom: '1rem' }}>
                ...
                {isAddingExercise && (
                  <form onSubmit={handleAddExercise} className="glass-panel" style={{ padding: '1rem', marginBottom: '1.5rem', background: 'rgba(255,255,255,0.01)' }}>
                    <h3 style={{ fontSize: '1rem', marginBottom: '0.8rem', color: 'var(--secondary)' }}>Agregar ejercicio</h3>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', marginBottom: '0.75rem' }}>
                      <div className="form-group" style={{ marginBottom: 0 }}>
                        ...
                        <input type="text" className="input-control" placeholder="Ej: 4x12, 3 series" value={newExerciseForm.Series_Repeticiones} onChange={e => setNewExerciseForm({...newExerciseForm, Series_Repeticiones: e.target.value})} />
                      </div>
                      <div className="form-group" style={{ marginBottom: 0 }}>
                ...
                <div className="space-y">
                  {uniqueDays.map(day => {
                    ...
                    return (
                      <div key={day} style={{ borderBottom: '1px solid var(--border)', paddingBottom: '1rem' }}>
                        <h3 style={{ fontSize: '1.1rem', color: 'var(--secondary)', marginBottom: '0.8rem', marginTop: '0.5rem' }}>
                          {day}
                        </h3>
                        <div className="space-y">
                                  ...
                                  <form onSubmit={handleSaveExerciseEdit} style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem' }}>
                                      <div>
                                        ...
                                        <input type="text" className="input-control" style={{ padding: '0.3rem' }} value={editExerciseForm.Series_Repeticiones || ''} onChange={e => setEditExerciseForm({...editExerciseForm, Series_Repeticiones: e.target.value})} />
                                      </div>
                                      <div>
                  ...
                  {studentRoutine.length > 0 && !isAddingExercise && (
                    <div style={{ marginTop: '1.5rem', background: 'rgba(255,255,255,0.01)', padding: '1rem', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)' }}>
            ...
            <div className="glass-panel" style={{ padding: '4rem 2rem', textAlign: 'center', color: 'var(--text-muted)', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
              <h3>Gestión de Rutinas Nexo Gym</h3>
      ...
      {isAddingStudent && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.7)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
          <div className="glass-panel fade-in" style={{ padding: '2rem', maxWidth: '600px', width: '100%', maxHeight: '90vh', overflowY: 'auto' }}>
            <h2 style={{ marginBottom: '1.5rem' }}>Registrar Nuevo Alumno</h2>
